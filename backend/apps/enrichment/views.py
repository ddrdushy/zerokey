"""Enrichment context views — Customers + Items UI surfaces.
  ...
  GET   /api/v1/items/                     — list items (most-used first)
  GET   /api/v1/items/<id>/                — item detail
  PATCH /api/v1/items/<id>/                — direct edits to an ItemMaster

The PATCH paths mirror the ``update_invoice`` editor: strict allowlist,
single audit event, alias filing on rename.
"""
...
def list_items(request: Request) -> Response:
    ...
    if not organization_id:
        return Response({"results": []})
    rows = services.list_item_masters(organization_id=organization_id)
    return Response({"results": ItemMasterSerializer(rows, many=True).data})
...
def item_detail(request: Request, item_id: str) -> Response:
    ...
    if request.method == "PATCH":
        return _item_update(request, organization_id, item_id)

    item = services.get_item_master(organization_id=organization_id, item_id=item_id)
    if item is None:
        return Response({"detail": "Not found."}, status=status.HTTP_404_NOT_FOUND)
    return Response(ItemMasterSerializer(item).data)


def _item_update(request: Request, organization_id: str, item_id: str) -> Response:
    if not isinstance(request.data, dict):
        return Response(
            {"detail": "Body must be a JSON object."}, status=status.HTTP_400_BAD_REQUEST
        )
    try:
        master = services.update_item_master(
            organization_id=organization_id,
            item_id=item_id,
            updates=request.data,
            actor_user_id=request.user.id,
        ...
        return Response({"detail": "Not found."}, status=status.HTTP_404_NOT_FOUND)
    except services.ItemUpdateError as exc:
        return Response({"detail": str(exc)}, status=status.HTTP_400_BAD_REQUEST)
    return Response(ItemMasterSerializer(master).data)
