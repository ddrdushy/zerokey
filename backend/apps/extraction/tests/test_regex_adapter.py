...
def test_extracts_invoice_number(structurer, target_schema) -> None:
    result = structurer.structure_fields(
        text=CLEAN_INVOICE_TEXT, target_schema=target_schema
    )
...
def test_extracts_total_with_thousand_separator(
    structurer, target_schema
) -> None:
...
def test_overall_confidence_scales_with_coverage(
    structurer, target_schema
) -> None:
    """Few hits = low overall; many hits = high overall."""
    sparse = "Date: 01/01/2026\n"
    dense = CLEAN_INVOICE_TEXT
    sparse_result = structurer.structure_fields(
        text=sparse, target_schema=target_schema
    )
    dense_result = structurer.structure_fields(
        text=dense, target_schema=target_schema
    )
